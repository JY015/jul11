...
DTO/VO = 
DAO = DB와 연결되어 작업한다. => mybatis
	
JSP = 보여주는 화면 (VIEW)
	
사용자 -> 컨트롤러 -> 서비스 -> DAO -> mybaits -> DB
사용자 <- JSP + DB <= 컨트롤러 <- 서비스 <- DAO <- mybaits <- DB

연결 = DI 의존 주입
1. @Autowired = 스프링에서 제공, 타입이 일치하는 녀석을 연결
2. @Inject	  = 자바에서 제공, 타입이 일치하는 녀셕을 연결
...
public class BoardController {
	...
	public String main() { // 위 문장과 연결되어서 /main 이라는 요청이 오면
